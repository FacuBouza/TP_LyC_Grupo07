...
if(@cant_types != @cant_var) return; @cant_types = @cant_var = 0;};
...
while_especial: WHILE ID IN exp_while DO bloque { idPointer = $2; while_especialPtr = crearNodo("WHILE_ESP", exp_whilePtr, bloquePtr);} ENDWHILE {printf("\nWHILE ID IN list_exp DO bloque ENDWHILE");};
exp_while: OP_CORC list_exp CL_COR {printf("\nOP_CORC list_exp CL_CO"); exp_whilePtr = list_expPtr;};
list_exp: expresion {printf("\nID"); condNodo = crearNodo("COND", crearNodo("==", idPointer, expresionPtr));} 
          | list_exp OP_COMA expresion {printf("\nlist_exp OP_COMA ID"); list_expPtr = crearNodo("COND", crearNodo("==", idPointer, expresionPtr), condNodo);};
...
for: FOR ID OP_ASIG asig_val { iniVar = crearNodo(":=", $2, asig_valPtr); } TO asig_val { condicionFor = crearNodo("<", $2, asig_valPtr); } pasos_for { stepNodo = crearNodo(":=", $2, pasos_forPtr); cuerpo2 = crearNodo("CUERPO", stepNodo, condicionFor); ini = crearNodo("INI", iniVar, cuerpo2); } bloque NEXT ID 
    {printf("\nFOR ID OP_ASIG asig_val TO asig_val pasos_for bloque NEXT ID");  forPtr = crearNodo("FOR", ini, bloquePtr);} 
    | FOR ID OP_ASIG asig_val { iniVar = crearNodo(":=", $2, asig_valPtr); } TO asig_val { condicionFor = crearNodo("<", $2, asig_valPtr); } bloque { stepNodo = crearNodo(":=", $2, 1); cuerpo2 = crearNodo("CUERPO", stepNodo, condicionFor); ini = crearNodo("INI", iniVar, cuerpo2); } NEXT ID {printf("\nFOR ID OP_ASIG asig_val TO asig_val bloque NEXT ID");  forPtr = crearNodo("FOR", ini, bloquePtr);} 
